...
Test-Command "docker-compose" "Docker Compose wird benoetigt"
...
$zipPath = "$env:TEMP\mitarbeiterverwaltung.zip"
...
$DbPassword = [Convert]::ToBase64String($bytes).Substring(0, 32)
...
if ($createCert -ne "n") {
    ...
    # OpenSSL verwenden falls vorhanden, sonst PowerShell
    if (Get-Command openssl -ErrorAction SilentlyContinue) {
        openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout certs\server.key -out certs\server.crt -subj "/CN=$ServerHost/O=IKK Kliniken/C=DE" 2>$null
    } else {
